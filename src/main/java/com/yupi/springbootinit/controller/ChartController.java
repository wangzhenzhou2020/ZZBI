...
//                "{明确的数据分析结论、越详细越好，不要生成多余的注释}";
        ...
        chart.setName(name);
        chart.setGoal(goal);
        chart.setChartData(csvData); // 这个数据有可能很大很大 新建一张表。开个线程
        ...
        CompletableFuture.runAsync(() -> {
            ExcelUtils.excelToDBTable(multipartFile, "chart_" + chart.getId());
        });
        // 返回结果
...
//                "{明确的数据分析结论、越详细越好，不要生成多余的注释}";
        ...
        try {
            ...
            try {
                ...
                if (b) {
                    throw new BusinessException(ErrorCode.SYSTEM_ERROR, "任务超时");
                } else {
                    throw new BusinessException(ErrorCode.SYSTEM_ERROR, "任务超时且未登记失败");
                }
            } catch (ExecutionException | InterruptedException e) {
                throw new BusinessException(ErrorCode.SYSTEM_ERROR,"任务未知异常");
            } finally {
            ...
            boolean b = chartService.recordChartFailed(chart); // 标记chart为失败
            if (b) {
                throw new BusinessException(ErrorCode.SYSTEM_ERROR, "任务繁忙");
            } else {
                throw new BusinessException(ErrorCode.SYSTEM_ERROR, "任务繁忙且未登记失败");
            }
        }
